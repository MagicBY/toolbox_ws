#!/usr/bin/env python
"""
Manipulability maximization position control for UR3 robot with torque-simulated rotational velocity tracking
"""
import swift
...
# Admittance parameters for rotation (simplified, steady-state)
D_d_rot = np.diag([10.0, 10.0, 10.0])  # damping matrix for rotation
inv_D_rot = np.linalg.inv(D_d_rot)
...
rot_vel_error_list = [] # ||J_r @ qd - omega_d|| (for S2 evaluation)
...
# Quaternion utility functions (kept for potential use, but not needed for velocity tracking)
def quat_mult(q1, q2):
    w1, x1, y1, z1 = q1
    w2, x2, y2, z2 = q2
...
while not arrived:
    try:
        ...
        # S2: Manipulability and Rotational Velocity Tracking
        Jm = ur3.jacobm(end=ur3.ee_links[0]).reshape((n,))
        I_n = np.eye(n)
        J_r = ur3.jacobe(ur3.q)[3:, :]
        J_t_pinv = np.linalg.pinv(J_t)
        N = I_n - J_t_pinv @ J_t
        q_ee = sm.UnitQuaternion(R_ee).vec  # Current quaternion (for reference)

        # Simulate external torque (customizable; here sinusoidal around y-axis)
        tau_ext = np.array([0.0, 1.0 * np.sin(2 * np.pi * time_elapsed / 10.0), 0.0])  # Nm, period 10s

        # Compute desired rotational velocity from simplified admittance
        omega_d = inv_D_rot @ tau_ext

        def objective_s2(q_dot_null):
            q_dot_total = q1 + N @ q_dot_null
            manip_term = 0.5 * np.linalg.norm(I_n @ q_dot_total - Jm)**2
            rot_term = lambda_penalty * np.linalg.norm(J_r @ q_dot_total - omega_d)**2
            return manip_term + rot_term
        ...
        # Evaluate S2: Compute rotational velocity error ||J_r @ qd - omega_d||
        rot_vel_error = np.linalg.norm(J_r @ qd - omega_d)
        rot_vel_error_list.append(rot_vel_error)
...
data_to_save = {
    ...
    "rot_vel_error_list": rot_vel_error_list
}
with open('manipulability_data_HMOC_updated.json', 'w') as f:
    json.dump(data_to_save, f)
print("Data saved to manipulability_data_HMOC_updated.json")